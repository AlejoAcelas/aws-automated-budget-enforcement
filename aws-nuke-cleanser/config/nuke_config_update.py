...
GLOBAL_RESOURCE_EXCEPTIONS = [
    ...
    {"property": "CreationDate", "type": "dateOlderThan", "value": "24h", "invert": "true"},
]
...
class StackInfo:
    ...
    def Populate(self):
        self.BuildIamExclusionList(self.regions) # Modify to pass self.regions
    ...
    def BuildIamExclusionList(self, regions):
    ...
    def OverrideDefaultConfig(self):
        ...
        try:
            with open(r"nuke_generic_config.yaml") as config_file:
                ...
                self.BuildIamExclusionList(self.config["regions"])
            ...
            for resource in self.config["resource-types"]['targets']:
                for exception in GLOBAL_RESOURCE_EXCEPTIONS:
                    if resource in self.config["accounts"][self.account]["filters"]:
                        self.config["accounts"][self.account]["filters"][resource].append(
                            exception.copy()
                        )
                    else:
                        self.config["accounts"][self.account]["filters"][resource] = [
                            exception.copy()
                        ]
            config_file.close()
    ...
    def WriteConfig(self):
        ...
        try:
            filename = "nuke_config.yaml"
            with open(filename, "w") as output_file:
                output = yaml.dump(self.config, output_file)
            output_file.close()
        except Exception as e:
...
if __name__ == "__main__":
    ...
    stackInfo = StackInfo(args.account)
    stackInfo.OverrideDefaultConfig()
